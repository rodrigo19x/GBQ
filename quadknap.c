...
#define MSIZE    200   /* maximum number of 0-1 variables */
...
void greedy(void)
{
    ...
    //xstar = (boolean*)malloc(sizeof(boolean)*n);
    for(i=0;i<n;i++) xstar[i] = x[i];
...
void quadbranch(stype ps, stype ws, int t)
{
    ...
    if((current_time-START) < LIMIT_TIME){
        //printf("Current Time %f sg\n",current_time-START);
        if ((t != n) && (ws + minw[t] <= c)) {
...
int checksolution(int **ptab,int *x_sol,int c, int z)
{
    int i, j;
    int psum, wsum, lprofit;
    ...
    return psum;
    //if (wsum > c) printf("excess weight\n");
    //if (psum != z) printf("bad solution\n");
}
/* ======================================================================
				quadknap
   ====================================================================== */

int quadknap(int no, int cap, int **ptab, int *wtab, int *xtab, double T_L)
{
    LIMIT_TIME = T_L;
    int cp_i;
    int cp_j;
    //printf("\n");
    for (cp_i = 0; cp_i < no; ++cp_i)
    {
        for (cp_j = 0; cp_j < no; ++cp_j)
        {
            ptab2[cp_i][cp_j] = ptab[cp_i][cp_j];
            //printf("%d ", ptab2[cp_i][cp_j]);
        }
        //printf("\n");
    }
    //printf("\n");
    ...
    //memcpy(p, ptab2, sizeof(int) * MSIZE * MSIZE);

    //memcpy(w, wtab, sizeof(int) * MSIZE);
    //w = (int *)malloc(sizeof(int)*no);
    //p = (int **)malloc(sizeof(int*)*no);
    for (cp_i = 0; cp_i < no; ++cp_i)
    {
        w[cp_i] = wtab[cp_i];
        //p[cp_i] = (int *)malloc(sizeof(int)*no);
        for (cp_j = 0; cp_j < no; ++cp_j)
        {
            p[cp_i][cp_j] = ptab2[cp_i][cp_j] ;
        }
    }
    ...
    for(cp_i=0;cp_i<no;cp_i++){
        xtab[cp_i] = xstar[cp_i];
    }
    //printf("%d\n",z );
    return checksolution(ptab,xtab,cap,z);
    //return z;
}
